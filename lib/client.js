...
function computeStats(session) {
  ...
  for (const n of nodes) {
    ...
    if (n.kind === 'user') {
      ...
      const c = countTextChars(n.content)
      toolChars += c
      if (JSON.stringify(n.content || []).indexOf(PRUNE_MARKER) >= 0) {
        prunedCount += 1
      }
  ...
  return {
    ...
    total, staticPct, requestCount, injectedToolCount, prunedCount,
...
function ContextConsole(props) {
  ...
  return React.createElement('div', { className: 'cctx-wrap' },
    ...
    React.createElement('div', { className: 'cctx-card' },
      React.createElement('div', { className: 'cctx-card-title' }, '结果修剪器'),
      React.createElement('div', { className: 'cctx-kv' },
        React.createElement('span', null, '已裁剪结果 ', React.createElement('b', null, s.prunedCount), ' 个'),
        React.createElement('span', null, '预算规则 ', React.createElement('b', null, '>8192 → head 4096 + tail 1024')),
      ),
      React.createElement('div', { className: 'cctx-hint' },
        '大工具结果由 host 端自动修剪（写入日志前生效），此处统计窗口内可见的已裁剪结果数。',
      ),
    ),
